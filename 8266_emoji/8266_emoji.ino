...
uint16_t convertColor(uint8_t r, uint8_t g, uint8_t b) {
  uint16_t tftcolor565 = tft.color565(r >> 3, g >> 2, b >> 3);
  return tftcolor565;
}
...
void webSocketEvent(uint8_t num, WStype_t type, uint8_t * payload, size_t length) {
  switch (type) {
    ...
    case WStype_TEXT:
      ...
      //char payload_c[12];
      //sprintf(payload_c,"%s",payload);
      Serial.println(); // 打印换行符，使输出更易读  
      //tft.print(payload_c);
      //OLED_SHOW();
      ...
      for (uint8_t y = 0; y < HEIGHT; y++) {
        for (uint8_t x = 0; x < WIDTH; x++) {
          ...
          uint8_t g = payload[i++];
          uint8_t b = payload[i++];
          tft.fillRect( 1+gap*x, 18+gap*y, 14, 14, tft.color565(r*10, g*20, b*10));
          Serial.printf("%x",tft.color565(r*565, g*565, b*565));
...
void setup() {
  ...
  //tft.print("init success!");
  //WiFiManager
  //Local intialization. Once its business is done, there is no need to keep it around
  WiFiManager wifiManager;
//  wifiManager.resetSettings(); //reset saved settings
  //wifiManager.setAPStaticIPConfig(IPAddress(192,168,174,100), IPAddress(192,168,174,1), IPAddress(255,255,255,0));
  //wifiManager.setSTAStaticIPConfig(IPAddress(192,168,174,199), IPAddress(192,168,174,1), IPAddress(255,255,255,0),IPAddress(8,8,8,8)); // optional DNS 4th argument
  wifiManager.autoConnect(MDNS_NAME);
  //tft.print(wifiManager.STAIP());
  // start MDNS
